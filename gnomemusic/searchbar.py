    ...
    def set_active(self, selected_id):
        ...
        if selected_value != []:
            ...
            if (selected_value[BaseModelColumns.ID] != 'search_all' and
                    selected_value[BaseModelColumns.ID] != (
                            'grl-tracker-source')):
                self.tag.set_label(
                    selected_value[BaseModelColumns.NAME])
...
class FilterView():
    ...
    def __init__(self, manager, dropdown):
        ...
        col.add_attribute(self._rendererText, 'text',
                          BaseModelColumns.NAME)
        col.set_cell_data_func(
            self._rendererText, self._visibilityForHeading, False)

        self.view.show()

    @log
    def _row_activated(self, view, path, col):
        id = self.model.get_value(
            self.model.get_iter(path), BaseModelColumns.ID)
...
class DropDown(Gtk.Revealer):
    ...
    def initialize_filters(self, searchbar):
        ...
        grilo.connect('new-source-added',
                      self.sourcesManager.add_new_source)
        grilo._find_sources()

        self.searchFieldsManager = BaseManager(
            'search', _("Match"), searchbar._search_entry)
        self.searchFieldsFilter = FilterView(
            self.searchFieldsManager, self)
...
class Searchbar(Gtk.SearchBar):
    ...
    def _drop_down_button_toggled(self, *args):
        self.dropdown.set_reveal_child(
            self._dropDownButton.get_active())

    @log
    def _search_entry_tag_button_clicked(self, entry, tag):
        tag.manager.set_active(tag.manager.values[1][
                               BaseModelColumns.ID])
    ...
    def search_entry_timeout(self, widget):
        ...
        self.timeout = GLib.timeout_add(
            500, self.search_entry_changed, widget)
