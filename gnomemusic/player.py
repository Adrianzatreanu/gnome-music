...
from gi.repository import (
    Gtk, Gdk, GLib, Gio, GObject, Gst, GstAudio, GstPbutils)
...
class Player(GObject.GObject):
    ...
    __gsignals__ = {
        ...
        'playlist-item-changed': (GObject.SignalFlags.RUN_FIRST, None,
    ...
    def __init__(self, parent_window):
        ...
        self.replaygain = self._settings.get_value(
            'replaygain') is not None
        self.toggle_replaygain(self.replaygain)

        self.bus.connect('message::state-changed',
                         self._on_bus_state_changed)
    ...
    def _check_last_fm(self):
        try:
            ...
            logger.info(
                "Error reading Last.fm credentials: %s" % str(e))
            self.last_fm = None
    ...
    def setup_replaygain(self):
        ...
        self._rglimiter = Gst.ElementFactory.make(
            "rglimiter", "rglimiter")
        self._rg_audiosink = Gst.ElementFactory.make(
            "autoaudiosink", "autoaudiosink")
        if not self._rgfilter or not (
            self._rg_audioconvert1) or not self._rg_audioconvert2 \
           or not self._rgvolume or not (
                self._rglimiter) or not self._rg_audiosink:
    ...
    def discover_item(self, item, callback, data=None):
        ...
        if not url.startswith("file://"):
            logger.debug(
                "Skipping discovery of %s as not a local file", url)
            return
    ...
    def _on_bus_state_changed(self, bus, message):
        ...
        # transitions between Gst.State.READY and Gst.State.NULL
        # are never async and thus don't cause a message
    ...
    def _gst_plugins_base_check_version(self, major, minor, micro):
        gst_major, gst_minor, gst_micro, gst_nano = (
				GstPbutils.plugins_base_version())
        return ((gst_major > major) or
                (gst_major == major and gst_minor > minor) or
                (gst_major == major and (
                    gst_minor == minor) and gst_micro >= micro) or
                (gst_major == major and (
                    gst_minor == minor) and (
                    gst_micro + 1 == micro) and gst_nano > 0))

    @log
    def _start_plugin_installation(self, missing_plugin_messages,
                                   confirm_search):
    ...
    @log
    def _show_codec_confirmation_dialog(self, install_helper_name,
                                        missing_plugin_messages):
        dialog = MissingCodecsDialog(
            self._parent_window, install_helper_name)

        def on_dialog_response(dialog, response_type):
            if response_type == Gtk.ResponseType.ACCEPT:
                self._start_plugin_installation(
                    missing_plugin_messages, False)
    ...
    def _handle_missing_plugins(self):
        ...
        if self._gst_plugins_base_check_version(1, 5, 0):
            proxy = Gio.DBusProxy.new_sync(
                Gio.bus_get_sync(Gio.BusType.SESSION, None),
                                ...
                                None)
            prop = Gio.DBusProxy.get_cached_property(
                proxy, 'DisplayName')
            if prop:
        ...
        # If the above failed, fall back to immediately starting the codec
        # installation
    ...
    def _onBusError(self, bus, message):
        ...
        debug = '\n'.join(debug)
        logger.warn('Error from element %s: %s',
    ...
    def _on_bus_eos(self, bus, message):
        if self.nextTrack:
            ...
            if self.playlist is not None:
                currentTrack = self.playlist.get_path(
                ...
                if currentTrack:
                    ...
                    self.currentTrackUri = self.playlist.get_value(
                        self.playlist.get_iter(self.currentTrack.get_path()),
                           5).get_url()
                else:
                    self.currentTrack = None
    ...
    def _get_random_iter(self, currentTrack):
        ...
        if hasattr(self.playlist, "iter_is_valid") and\
           not self.playlist.iter_is_valid(currentTrack):
            return None
        currentPath = int(self.playlist.get_path(
    ...
    def _get_next_track(self):
        ...
        if nextTrack:
            return Gtk.TreeRowReference.new(self.playlist,
    ...
    def _get_previous_track(self):
        ...
        if self.repeat == RepeatType.SONG:
            ...
            if currentTrack:
                previousTrack = self.playlist.iter_previous(
                    currentTrack)
            ...
            if currentTrack:
                if self.played_seconds < 10 and len(self.shuffleHistory) > 0:
                    ...
                    if self.playlist.get_path(previousTrack) == (
                            self.playlist.get_path(currentTrack)):
                        previousTrack = None
    ...
    def has_next(self):
        if not self.playlist or self.playlist.iter_n_children(None) < 1:
            ...
            return False
        elif self.repeat in [RepeatType.ALL, RepeatType.SONG,
                             RepeatType.SHUFFLE]:
            return True
    ...
    def _get_playing(self):
        ...
        # log('get playing(), [ok, state, pending] =  \
    # [%s, %s, %s]'.format(ok, state, pending))
    ...
    def load(self, media):
        ...
        self.cache.lookup(media, ArtSize.xsmall,
                          self._on_cache_lookup, None)
        ...
        if self.currentTrack and self.currentTrack.valid():
            currentTrack = self.playlist.get_iter(
                self.currentTrack.get_path())
            self.emit('playlist-item-changed',
                      self.playlist, currentTrack)
    ...
    def _on_next_item_validated(self, info, error, _iter):
        if error:
            ...
            self.playlist.set_value(
                _iter, self.discovery_status_field, DiscoveryStatus.FAILED)
            nextTrack = self.playlist.iter_next(_iter)

            if nextTrack:
                self._validate_next_track(Gtk.TreeRowReference.new(
                    self.playlist, self.playlist.get_path(nextTrack)))
    ...
    def _validate_next_track(self, track=None):
        ...
        status = self.playlist.get_value(
            _iter, self.discovery_status_field)
        ...
        if url.startswith('http://') or url.startswith('https://'):
            ...
            self.discover_item(
                nextSong, self._on_next_item_validated, _iter)
    ...
    def _setup_view(self):
        ...
        self.coverImg.set_property(
            "width-request", ArtSize.xsmall.width)
        self.coverImg.set_property(
            "height-request", ArtSize.xsmall.height)
    ...
    def _on_progress_value_changed(self, widget):
        ...
        self.songPlaybackTimeLabel.set_label(
    ...
    def scrobble_song(self, url):
        ...
        if self.last_fm:
            ...
            sig = "api_key%sartist[0]%smethodtrack.scrobblesk" + \
                "%stimestamp[0]%strack[0]%s%s" %\
                    (api_key, self._currentArtist, sk,
                     self._currentTimestamp, self._currentTitle, secret)
            ...
            try:
                r = requests.post(
                    "https://ws.audioscrobbler.com/2.0/", requests_dict)
    ...
    def update_now_playing_in_lastfm(self, url):
        if self.last_fm:
            ...
            sig = "api_key%sartist%smethodtrack." + \
                "updateNowPlayingsk%strack%s%s" % \
                   (api_key, self._currentArtist,
                    sk, self._currentTitle, secret)
        ...
        if seconds != self.duration:
            self.player.seek_simple(
                Gst.Format.TIME, Gst.SeekFlags.FLUSH | (
                    Gst.SeekFlags.KEY_UNIT), seconds * 1000000000)
            try:
            ...
            if duration:
                ...
                try:
                    self.emit(
                        'seeked', (duration[1] - 1000000000) / 1000)
                except TypeError:
                    # See
    ...
    def get_current_media(self):
        ...
        currentTrack = self.playlist.get_iter(
            self.currentTrack.get_path())
        if self.playlist.get_value(currentTrack,
                                   self.discovery_status_field) == (
                            DiscoveryStatus.FAILED):
            return None
...
class MissingCodecsDialog(Gtk.MessageDialog):
    ...
    def __init__(self, parent_window, install_helper_name):
        ...
        self.find_button = self.add_button(
                _("_Find in %s") % install_helper_name,
...
class SelectionToolbar():
    ...
    def __init__(self):
        ...
        self._ui.add_from_resource(
            '/org/gnome/Music/SelectionToolbar.ui')
        self.actionbar = self._ui.get_object('actionbar')
        self._add_to_playlist_button = self._ui.get_object('button1')
        self._remove_from_playlist_button = self._ui.get_object(
            'button2')
